#' @export
summarize_results_power = function(path){
...
for (m in methods){
  ...
  # Load results file 
  dir1<-dir(paste(path,"\\Results/SimRes_",m,"_",d,sep=""))
  
  
  file1<-paste(path,"\\Results/SimRes_",m,"_",d,
...
alpha=0.05 
colnr= grep(as.numeric(alpha),colnames(results))
...
results_datagen[[d]]=result_method_datagen
}

save(results_datagen,file=paste0(path,'\\Results_power_perdatagen.RData'))


## Summarize power results per datageneration method & without scenario settings in 1 dataframe

results_datagen_df=data.frame()
...
for (d in data.gen){
  ...
  results_datagen_df=rbind(results_datagen_df,result_method_datagen_tmp)
}

save(results_datagen_df,file=paste0(path,'\\Results_power_perdatagen_df.RData'))
