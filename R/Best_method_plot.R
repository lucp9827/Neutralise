...
Best_method_plot = function(path,name_extra,n=20,alpha=0.05,name_methods=NULL,N=10000){
  ...
  txt=paste(name_extra,'has the largest power in',sum(tt1$power.x>tt1$power.y,na.rm=TRUE),'of the ',length(tt1$power.x),'scenarios, which is ',round(sum(tt1$power.x>tt1$power.y,na.rm=TRUE)/length(tt1$power.x),digits=2)*100,"% of the scenarios.")

  hh=tt1[(tt1$power.x<tt1$power.y),]
  hh$difference = hh$power.y-hh$power.x
  median(hh$difference)

  txt2 = paste("The median of the power differences for scenarios where ",name_extra," has smaller power than the best test is ",round(median(hh$difference,na.rm=TRUE),digits = 2))
