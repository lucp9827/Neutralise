...
NeutraliseFiles contains 5 directories;
...
To run all combinations of statistical methods and simulation scenarios (i.e. combination of data generator and settings) available on Github the following code can be used and the results will be saved in the *Results* directory. 


This function can be used to reproduce results of previous simulations. Note: To reproduce results it's important to have (1) an 'empty' *Results* directory and (2) to reinitialise the files *Finished.txt* and *neutralise_status.Rfile*. Also (3) add the specific settings RData-files that you want to reproduce into the *Settings* directory.

```{r}

# Code to (re)initilize the files *Finished.txt* and *neutralise_status.Rfile (dont run if you don't want to reproduce all results)
...
### Summarize results power & Type 1 error 

This might take a little while (10-20 minutes), depending on the amount of results.The summarized results are also available in the zipfile with all the results, it isn't necessary to run this if you didn't added new results to the Neutralise files. 
...
Graph1_type1error = Boxplot_TypeI(path,method1,alpha=0.05)
...
Graph4_type1error = Boxplot_TypeI(path,method=NULL,panel="distribution")
...
method1=methods[1] # CVM
method2=methods[6] # "TTest_VarUnequal"
...
Graph2 = Power_QQ(path,method1,method2,alpha=0.05,group=TRUE)
...
Graph3 = Power_QQ(path,method1,method2,alpha=0.05,group=TRUE,par.fix=db)
...
method=methods[2] # CVM
data=data.gen[2]


# Power curve - (not clear if scenarios has repetitions of delta, solution see next step )

Graph1 = Power_curve(path,method,data,alpha=0.05)

Graph1

# Power curve and a specified setting parameter (rate1 = 0.5)

db=data.frame(rate1=0.5) # important that the specified parameter is exactly the same as in the setting file 

Graph2 = Power_curve(path,method,data,alpha=0.05,par.fix = db,CI=TRUE)
...
Best_method_plot(path,name_extra,n=20,alpha=0.05,name_methods=NULL)
```
